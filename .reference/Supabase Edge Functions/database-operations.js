...
const DELETION_CONFIG = {
  ...
  tablesToDelete: [
    ...
    'user_settings',         // User settings and tokens
    // Add new user data tables here as you create them:
    // 'user_calendar_cache',
    // 'user_preferences',
    // 'user_custom_widgets',
  ],
...
async function handleDeleteAccount(supabase, userId, data) {
  ...
  try {
    ...
    if (DELETION_CONFIG.enableDiscoveryMode) {
      ...
      const notInConfig = discoveredTables.filter(
        table => !configuredTables.includes(table)
      );
