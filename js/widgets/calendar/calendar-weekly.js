// js/widgets/calendar/calendar-weekly.js - Weekly View Rendering Module
...
const logger = createLogger('CalendarWeekly');
...
export class CalendarWeekly {
  ...
  /**
   * Generate background style for multi-calendar events (split colors)
   * Creates a diagonal split like Skylight Calendar
   * @param {Object} event - Event object
   * @returns {string} CSS background style
   */
  getEventBackgroundStyle(event) {
    // Single calendar event - use solid color
    if (!event.isMultiCalendar || !event.calendars || event.calendars.length === 1) {
      const backgroundColor = event.backgroundColor ||
                             this.calendars.find(cal => cal.id === event.calendarId)?.backgroundColor ||
                             '#1976d2';
      return backgroundColor;
    }

    // Multi-calendar event - create diagonal gradient split
    const colors = event.calendars.map(cal => cal.backgroundColor);
    const count = colors.length;

    if (count === 2) {
      // Two calendars: VERTICAL split line with slight slant (Skylight style)
      // Line tilts from 60% (top) to 40% (bottom) as it goes down
      // 100deg = nearly vertical split, slanting left
      return `linear-gradient(100deg, ${colors[0]} 50%, ${colors[1]} 50%)`;
    } else {
      // 3+ calendars: vertical stripes with slight diagonal
  ...
  renderEvents(calendarData) {
    ...
    logger.debug('Events rendered in weekly view', {
  ...
  renderAllDayEvents(allDayEvents) {
    ...
    eventRows.forEach((row, rowIndex) => {
      row.forEach(eventInfo => {
        ...
        // Apply dynamic colors (solid or split for multi-calendar events)
        const backgroundStyle = this.getEventBackgroundStyle(event);
        if (backgroundStyle.startsWith('linear-gradient')) {
          eventElement.style.background = backgroundStyle; // Use 'background' for gradients
        } else {
          eventElement.style.backgroundColor = backgroundStyle; // Use 'backgroundColor' for solid
        }
