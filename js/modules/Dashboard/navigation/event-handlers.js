 ...
 *
 * Handles clicks on the focus overlay and outside the sidebar menu:
 * - Click outside sidebar (when menu open): Close menu
...
class OverlayEventHandler {
  ...
  static attach(container) {
    ...
    // CRITICAL: Detect clicks on widget iframes (clicks inside iframes don't bubble up)
    // When an iframe gets focus, it means the user clicked on it
  ...
  /**
   * Handle click on overlay or outside focused widget/menu
   * @param {Event} e - Click event
   */
  static handleClick(e) {
    ...
    // Handle clicks when widget is focused
    if (state.focusedWidget) {
